...
loadLanguages();
const codeTag = (s) => `<code>${s}</code>`;
const mustache = (s) => `{@html \`${s}\` }`;
const isCodeTag = (n) => n.type === 'Element' && n.name === 'code';
const isMustacheTag = (n) => n.type === 'MustacheTag';
const sveltePreprocessPrism = {
    name: 'svelte-prism',
    markup({ content, filename }) {
        const ms = new MagicString(content);
        const ast = parse(ms.toString());
        walk(ast.html, {
            enter(node) {
                if (node.type === 'Element' && node.name === 'pre') {
                    ...
                    ms.overwrite(node.children[0].start, node.children[node.children.length - 1].end, tagged);
                }
            },
        });
        return {
